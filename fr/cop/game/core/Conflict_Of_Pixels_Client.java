...
public class Conflict_Of_Pixels_Client extends Canvas implements Runnable {
	...
	public static void main(String[] args) { // Methode de demarrage d'un
		...
		menuFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // On ferme le
																	// programme
																	// lorsque
																	// la fentre
																	// est
	...
	public synchronized void start() { // Fonction de démarrage du jeu.
		...
		CHARACTER_LIST = new CharacterList(); // On instancie notre liste de
												// champions.
		t = new Thread(this, "CoP"); // On créer le Thread du jeu. (pas celui
										// de l'affichage).
	...
	public synchronized void visualUpdate() { // Methode actualisant
		...
		// joueur.
		BufferStrategy bs = getBufferStrategy(); // On récupère la stratégie
													// de buffer.
		...
		Graphics g = bs.getDrawGraphics(); // On récupère les graphics de
											// notre canvas.
		...
		if (debug) { // Si le debug est activé. On affiche les options de
						// debug. (Modifié plus tard pour avoir plus bel
						// affichage, [fenêtre externe ?]).
