...
public class Conflict_Of_Pixels_Client extends Canvas implements Runnable {
	...
	private boolean running = false; // Boolean permettant de dire si le jeu
										// fonctionne ou non.
	public boolean isGamePaused = false; // Permet de mettre le jeu en pause. Ne
											// sera utilisable que depuis le
											// serveur (� terme).
	...
	public static int height = width / 16 * 9; // Taille de la fen�tre
												// (hauteur).

	private int imageRenderedWidth = width * scale, imageRenderedHeight = height * scale; // Taille
																							// de
																							// rendu
																							// de
																							// l'image
	...
	public static Screen screenGAME; // Notre ecran de jeu, permettant le rendu
										// (pixel � pixel).
	public HUD hud;
	...
	public Conflict_Of_Pixels_Client() { // Objet etant notre jeu.
		...
		debugWindow = new SimpleDebugWindow(); // Cr�ation de notre fen�tre de
												// debug.
		serverGame = new Game();
	}
	...
	public synchronized void start() { // Fonction de d�marrage du jeu.
		...
		t = new Thread(this, "CoP"); // On cr�er le Thread du jeu. (pas celui de
										// l'affichage).
	...
	public void run() { // Exécution de notre jeu.
		...
		debugWindow.changePauseStatue(); // On affiche l'�tat de la pause dans
											// la fen�tre de debug.
		while (running) { // Boucle principale de notre programme.
			...
			if (currentTime >= startTimeFPS + nbFps || nbFps < 0) { // Si le
																	// temps du
																	// processeur
																	// est
																	// sup�rieur
			...
			if (System.currentTimeMillis() - timer > 1000) { // Si le jeu passe
																// 1 seconde.
	...
	public synchronized void gameUpdate() { // Methode visant l'actualisation du
		...
		/*
		 * TO-DO : - Utilisation de la souris. - Envoie des packets au server. -
		...
		if (keyboard.keys[KeyEvent.VK_P] && keyboard.canPressP && !isGamePaused)
			pause(); // Si on appuie sur 'P', que l'on peut mettre/enlever la
						// pause et qu'on est pas d�j� en pause, on met la
						// pause.
		else if (keyboard.keys[KeyEvent.VK_P] && keyboard.canPressP && isGamePaused)
			backToTheGame(); // Si on appuie sur 'P', que l'on peut
								// mettre/enlever la pause et qu'on est d�j� en
								// pause, on enl�ve la pause.

		if (!isGamePaused) { // Si le jeu n'est pas en pause.
			if (keyboard.directions[0])
				y += cameraSpeed; // On d�place la cam�ra en direction du haut.
			if (keyboard.directions[1])
				y -= cameraSpeed; // On d�place la cam�ra en direction du bas.
			if (keyboard.directions[2])
				x += cameraSpeed; // On d�place la cam�ra en direction de la
									// droite.
			if (keyboard.directions[3])
				x -= cameraSpeed; // On d�place la cam�ra en direction de la
									// gauche.

			debugWindow.setDirectionKeysState(keyboard.directions); // On
																	// actualise
																	// l'�tat
																	// des
																	// touches
																	...
																	// fen�tre
																	// de debug.
			debugWindow.setItemsKeysState(keyboard.items); // On actualise
															// l'�tat des
															// touches des items
															// dans le fen�tre
															// de debug.
			debugWindow.setSpellsKeysState(keyboard.spells); // On actualise
																// l'�tat des
																// touches des
																// sorts dans la
																// fen�tre de
			...
			screenGAME.increaseTimer(); // On incr�mente le timer de notre
										// screen, permet d'avoir des
										// animations.
			// serverGame.getChampion(0).move();
		}
	}

	public synchronized void visualUpdate() { // Methode actualisant l'ecran.
												// Pas le jeu. Peut etre
												// different entre chaque
												// joueur.
		BufferStrategy bs = getBufferStrategy(); // On r�cup�re la strat�gie de
													// buffer (du canvas).
		if (bs == null) { // Si elle n'�xiste pas ...
			createBufferStrategy(3); // ... on met du triple Buffering. (3
										// images tampons).
	...
	public synchronized void pause() { // M�thode pour mettre en pause le jeu.
		...
		keyboard.canPressP = false; // On oblige le relachement de la touche de
									// pause avant de l'enlever.
		debugWindow.changePauseStatue(); // On affiche l'�tat de la pause dans
											// la fen�tre de debug.
	}
	...
	public synchronized void backToTheGame() { // M�thode pour enlever la pause.
		...
		keyboard.canPressP = false; // On oblige le relancement de la touche de
									// pause avant de la remettre.
		debugWindow.changePauseStatue();// On affiche l'�tat de la pause dans la
										// fen�tre de debug.
	}

	public synchronized void pauseBeforeChange() { // M�thode pour mettre le
													// Thread en pause.
		running = false; // On arr�te la boucle principale.
		try { // On essaie de ...
			t.join(); // ... arr�ter le Thread.
		} catch (InterruptedException e) { // Si une erreur est lev�e...
			e.printStackTrace(); // ... on �crit le rapport d'erreur dans la
									// console.
		}
	}

	public synchronized void continueAfterChange() { // M�thode pour relance le
														// Thread
	...
	public boolean getDebugState() { // M�thode pour savoir si le debug est
										// activ�.
		return debug;
	}

	public void setDebugState(boolean debugState) { // M�thode pour activer ou
													// non le debug sur la
													// fenetre de jeu.
		debug = debugState;
	}

	public void setFpsLimitatiob(int fps) { // M�thode pour changer la
											// limitation de FPS
		nbFps = 1000 / fps;
	}

	public synchronized void setImageRenderedSize(int width, int height) { // M�thode
																			// pour
																			// changer
																			// la
																			// taille
